...
TRACE_CLIENT = Client(
    ...
    retry_config=Retry(total=5, connect=5, read=5, backoff_factor=1),
)


# ── 1. Tên Prompt trên Hub ─────────────────────────────────────────────────
# Tên Prompt Hub duy nhất cho bài lab.
PROMPT_V1_NAME = "truong-minh-hoang-day22-rag-v1"
PROMPT_V2_NAME = "truong-minh-hoang-day22-rag-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
# V1 ưu tiên câu trả lời ngắn gọn trong 2-4 câu.
# Gợi ý: "Bạn là trợ lý AI hữu ích. Chỉ dùng context sau để trả lời.
#          Giữ câu trả lời ngắn gọn (2-4 câu). ..."
SYSTEM_V1 = (
    "Bạn là trợ lý AI hữu ích. Chỉ trả lời bằng thông tin trong context. "
    "Nếu context không đủ, hãy nói rõ điều đó. Trả lời ngắn gọn trong 2-4 câu.\n\n"
    "Context:\n{context}"
)
...
# V2 dùng giọng chuyên gia và cấu trúc rõ ràng trong 3-5 câu.
# Gợi ý: "Bạn là chuyên gia AI. Đọc kỹ context, xác định facts liên quan,
#          viết câu trả lời rõ ràng và có tổ chức (3-5 câu). ..."
SYSTEM_V2 = (
    "Bạn là chuyên gia AI. Hãy đọc kỹ context, chọn các dữ kiện liên quan và "
    "trả lời rõ ràng, có cấu trúc trong 3-5 câu. Không thêm kiến thức ngoài context; "
    "nếu thiếu dữ kiện, hãy nêu giới hạn của câu trả lời.\n\nContext:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME, object=PROMPT_V1, description="V1 - trả lời ngắn gọn 2-4 câu"
        )
    ...
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME, object=PROMPT_V2, description="V2 - chuyên gia, trả lời có cấu trúc"
        )
...
# ── 6. Traced A/B Query ────────────────────────────────────────────────────
@traceable(name="ab-rag-query", tags=["ab-test", "step2"], client=TRACE_CLIENT)
        ...
        for attempt in range(1, 4):
            try:
                ...
                if attempt == 3:
                    raise
                delay = 2 ** attempt
                print(f"  ⚠️  Lỗi mạng ({exc}); thử lại sau {delay}s ...")
                time.sleep(delay)
