...
class PIIDetector(Validator):
    ...
    # Regex cho bốn nhóm PII được kiểm tra và che tự động.
    PII_PATTERNS = {
    ...
    def validate(self, value: str, metadata: dict):
        ...
        if found_pii:
            ...
            # Trả FailResult để OnFailAction.FIX thực sự áp dụng fix_value.
            return FailResult(
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        ...
        result = guard.validate(text)
        assert result.validated_output is not None
        parsed_output = json.loads(str(result.validated_output))
        if label == "Truly invalid":
            assert parsed_output["error"] == "Không thể phân tích JSON"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chạy demo Guardrails Day 22")
    parser.add_argument(
        "--demo", choices=["all", "pii", "json"], default="all",
        help="Chọn nhóm demo để lưu log minh chứng riêng.",
    )
